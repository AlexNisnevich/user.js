...
// Everything else goes into this method
function main() {
	...
	var $log = $('<div id="log" style="display: none;">');
	var results = {};
	var resultsArray = [];

	function processLink(link) {
		var url = link.href;
		if (url.match('ludum-dare-[0-9]*/')) {
			var compoNumber = 'LD' + url.match('ludum-dare-([0-9]*)')[1];
			results[compoNumber] = {};
			...
			$.get(url, function(data) {
				...
				drawVisualization(resultsToArray(results));
			});
		}
	}
	...
	function resultsToArray(results) {
		...
		$.each(results, function (num, scores) {
			$.each(scores, function (category, score) {
		...
		$.each(results, function (num, scores) {
			...
			$.each(scores, function (category, score) {
				row[categories.indexOf(category)] = score;
			});
	...
	function drawVisualization(resultsArray) {
	  $('#visualization').html('');

	  console.log(JSON.stringify(resultsArray));

	  // Create and populate the data table.
	  var data = google.visualization.arrayToDataTable(resultsArray);
	...
	$($('#compo2 a').get().reverse()).each(function(i, link) {
		processLink(link);
	});

	var username = $($('h2.pagetitle')[2]).first().text().match('(.*)\'s')[1];
	$('#compo2').after($('<div><h2 class="pagetitle">' + username + '\'s Performance</h2><div class="post" id="visualization"></div></div>'));
}
